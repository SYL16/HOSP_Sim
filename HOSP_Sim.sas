...
*              alpha & beta : parameters of the Erlang distribution for length of hospital stay, x = beta*RAND('ERLANG',alpha)
*                                                                                   
* Output:                                                                           
...
%macro hosp_sim(nSim=100, dMoni=120, inc=200, incSF=1.22, pHosp=0.1, dInf2Hosp=14, alpha=2, beta=4);  
options nonotes;
  data sim_hosp_cases;
...
proc sgplot data=beds_comp; 
	 ...
	 keylegend 'groups' / type=linecolor position=bottom title="Daily new cases in Shelby County";
	 title "Number of hospital bed census in the Memphis metro area (100 simulations)";
	 xaxis values=(0 to 120 by 10);
	 yaxis label="Beds" grid minor values=(0 to 1000 by 100);
run;
...
proc sgplot data=stats; 
	 ...
	 keylegend / type=fillcolor position=bottom title="Daily new cases in Shelby County";
	 title "Estimated number of hospital bed census in the Memphis metro area (100 simulations)";
	 xaxis values=(0 to 120 by 10);
	 yaxis label="Beds" grid minor values=(0 to 1000 by 100);
run;
